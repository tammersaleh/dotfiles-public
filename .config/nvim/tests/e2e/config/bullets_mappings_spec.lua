...
describe("bullets.vim mappings in markdown (with plugins)", function()
  ...
  describe("inside a blockquote", function()
    it("toggles a quoted checkbox with <leader>x", function()
      h.set_buf({ "> - [ ] task" })
      h.set_cursor(1)
      h.feed("<leader>x")
      assert.are.same({ "> - [x] task" }, h.get_buf())
    ...
    it("renumbers a quoted list with gN", function()
      h.set_buf({ "> 5. one", "> 8. two" })
      h.set_cursor(1)
      h.feed("gN")
      assert.are.same({ "> 1. one", "> 2. two" }, h.get_buf())
    ...
    it("renumbers a quoted visual selection with gN", function()
      h.set_buf({ "> 5. one", "> 8. two", "> 9. three" })
      h.set_cursor(1)
      h.feed("Vj")
      h.feed("gN")
      assert.are.same({ "> 1. one", "> 2. two", "> 9. three" }, h.get_buf())
    end)

    it("continues a quoted list with o", function()
      h.set_buf({ "> - one" })
      h.set_cursor(1)
      h.feed("otwo")
      h.ensure_normal()
    ...
    it("continues a quoted paragraph with o", function()
      h.set_buf({ "> one" })
      h.set_cursor(1)
      h.feed("otwo")
      h.ensure_normal()
    ...
    it("continues a nested quoted paragraph with o", function()
      h.set_buf({ "> > one" })
      h.set_cursor(1)
      h.feed("otwo")
      h.ensure_normal()
    ...
    it("o on a plain paragraph is still a plain new line", function()
      h.set_buf({ "one" })
      h.set_cursor(1)
      h.feed("o") -- bullets.vim queues the plain <cr>, so type nothing after
      h.ensure_normal()
